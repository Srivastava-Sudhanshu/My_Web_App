...
from My_Web_App.settings import Home,DUE_EMAIL_TEMPLATE
...
#from task import DueFeeNotification as dfn
...
def PayFee(request, id):
    ...
    if request.method == "POST":
        ...
        if studentfee.fees_paid == current_year_fee.fee:
            ...
            form = GetStudentFeeDetailsForm(current_year_fee, studentfee)
    else:
        form = GetStudentFeeDetailsForm(current_year_fee, studentfee)
...
def GetStudentFeeDetailsForm(current_year_fee, studentfee):
    studentfee.due_amount = int(current_year_fee.fee) - int(studentfee.fees_paid)
    result = StudentFeeDetailsForm(instance=studentfee)
    result.fields['payment_status'].widget.attrs['disabled'] = True
    return result
